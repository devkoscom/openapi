...
### Wavelet <a id="wavelet-api"></a>

* 코스피/코스닥 상장 기업들에 대한 실시간 시세 및 과거 시세 및 거래량 정보를 조회
* RESTFUL API 의 형태로 호출하며, 사용자 입장에서 시세와 관련된 별도의 서버를 구축할 필요가 없다.
* 다수의 종목을 동시에 조회할 수 있다.
* 1985년 부터 현재까지의 시세 및 거래량을 조회할 수 있다.
* 연간, 월간, 일간, 분간 간격으로 데이터의 조회가 가능하며, 사용자가 임의로 간격을 지정할 수 있다. \( ex\) 32m : 32분 간격 \)
* 시세, 거래량 및 종목의 간단한 정보\(액면가, 자본금, PBR, PER 등\) 을 확인 할 수 있다.
...
* RESTFUL API 의 형태로 호출하며, 사용자 입장에서 재무정보와 관련된 별도의 서버를 구축할 필요가 없다.
* 다수의 종목을 동시에 조회할 수 있다.
* 1996년 부터 현재까지의 재무정보\(GAAP, IFRS, 연결, 별도\) 를 조회할 수 있다.​
...
* 다양한 검색어에 따라 뉴스 및 분석 문서 검색 제공
* 특정 검색어의 시간대별 트렌드를 계산하여 검색 제공
* 현재 시장에서 이슈가 되고 있는 주제 및 관련 문서를 추출하여 제공
* 특정 주제와 관련된 기업 및 관련 근거를 제공
